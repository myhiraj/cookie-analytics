...
ANALYSIS_METHODS = ["most_active_cookie_for_date"]
...
def main():
    ...
    parser.add_argument("log_file_path", required=False, help="Path to the cookie log file (CSV format)")
    ...
    if args.method_selection == "most_active_cookie_for_date":
        if not args.log_file_path or not args.d:
            ...
            most_active_cookies = cookie_log.most_active_cookie(target_date)
        ...
        if args.method_selection == "most_active_cookie_for_date_range":
            if not args.log_file_path or not args.start_date or not args.end_date:
                print("For 'most_active_cookie_for_date_range', 'log_file_path', '--start_date', and '--end_date' arguments are required.")
                sys.exit(1)
            else:
                ...
                most_active_cookies = cookie_log.most_active_cookie(start_date, end_date)
